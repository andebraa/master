...
for seed in [53538]:
    ...
    if grid: #system might not be erratic, but it could still be grid 
        ...
        if erratic:
            print(push_dir + f"sim_temp{temp}_vel{vel}_force{force}_time{push_time}_seed{seed}_errgrid{grid[0]}_{grid[1]}")
            
            sim = Simulator(directory=push_dir + f"sim_temp{temp}_vel{vel}_force{force}_time{push_time}_seed{seed}_errgrid{grid[0]}_{grid[1]}", overwrite=True)

        else:
            sim = Simulator(directory=push_dir + f"sim_temp{temp}_vel{vel}_force{force}_time{push_time}_seed{seed}_grid{grid[0]}_{grid[1]}", overwrite=True)
    

    else:
        sim = Simulator(directory=push_dir + f"sim_temp{temp}_vel{vel}_force{force}_time{relax_steps}_seed{seed}", overwrite=True)
    
    sim.copy_to_wd(restartfile, lammps_dir + "SiC.vashishta", "sigmoid.py")
    sim.set_input_script(lammps_dir + "in.push", **var)
    sim.run(computer=SlurmGPU(lmp_exec="lmp_python", slurm_args={'job-name': f'{int(temp/100)}_{relax_steps}_{seed}_erratic{erratic}'}, lmp_args={'-pk': 'kokkos newton on neigh full'}))
