...
class GridSearchDNN(GridSearch):
    ...
    def _fit(self, params, X, y, epochs, kfold_splits):
        ...
        for train_idx, val_idx in KFold(n_splits=kfold_splits, random_state=0, shuffle=True).split(X):
            ...
            true_train, pred_train = utils.test_model(model, self.device, self.criterion, train_loader, plot_predictions=True, verbose=True, title="train")
            true_val, pred_val = utils.test_model(model, self.device, self.criterion, val_loader, plot_predictions=True, verbose=False, title="val")
...
def run_dnn_search(epochs, mode):
    ...
    n_nodes_list = 2**np.arange(2,3) # 4 - 1024 nodes
    n_layers_list = 2**np.arange(2,3) # 2 - 128 layers
    ...
    test_true, test_pred = utils.test_model(model, device, nn.MSELoss, test_loader, title="test", savefig = 'test_fig.png')
